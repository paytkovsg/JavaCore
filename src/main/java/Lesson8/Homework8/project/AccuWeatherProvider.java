...
public class AccuWeatherProvider implements WeatherProvider, DatabaseRepository {
    ...
    public void getWeatherBase(Periods periods) throws IOException, SQLException {
        ...
        if (objectMapper.readTree(jsonResponse).size() > 0) {
            ...
            double temperature = objectMapper.readTree(jsonResponse).get(0).at("/Temperature/Metric/Value").asDouble();
            ...
            //Добавляем данные из ответа в базу
            databaseRepositorySQLite.createTableIfNotExists();
            databaseRepositorySQLite.saveWeatherData(ApplicationGlobalState.getInstance().getSelectedCity(),
                    dateTime, weatherText, temperature);

            System.out.println("Занесено в базу данных");
            databaseRepositorySQLite.selectAll();
    ...
    public void getWeather(Periods periods) throws IOException, SQLException {
        ...
        switch (periods) {
            case NOW:
                ...
                List<WeatherResponse> weatherResponses = objectMapper.readValue(jsonResponse,
